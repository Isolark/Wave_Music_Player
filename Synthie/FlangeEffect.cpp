...
CFlangeEffect::CFlangeEffect(const double &delayMean, const double &delayAmplitude, const double &delayFrequency)
{
...
void CFlangeEffect::Process(double * const &frame)
{
	// advance the wrloc
	...
	{
		frame[i] =  0.5 * (frame[i] + m_queue[rdloc + i]);
	}
...
void CFlangeEffect::Factory::XmlLoad(IXMLDOMNode * xml)
{
